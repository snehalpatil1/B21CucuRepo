...
public class MyAuthenticationstepdef {
	...
	public void user_hit_the_get_request_with_username_and_password(){
		//Response resp = 
		resp = RestAssured.given().auth().basic("username", "password").relaxedHTTPSValidation(). //we used basic authenticayion code if its not working then we have to communicate with developer for authentication code validation which they are using
