...
public class MyPostStringJson {
	...
	public void user_hit_the_post_request_with_String() throws IOException{
		...
		resp = RestAssured.given().accept(ContentType.JSON).relaxedHTTPSValidation().               //Provide to ignore http validation
	...
	public void user_validate_the_id_is_not_null(){
		...
		//Validate the date in createdAt

		@Then("^user validate the today date$")
		public void user_validate_the_today_date() {
			...
			Assert.assertTrue(createdAtValue.contains(todayDate));
		}
